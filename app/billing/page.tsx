...
export default function BillingPage() {
  ...
  useEffect(() => {
    const sessionStr = localStorage.getItem("module_session_billing")
    if (!sessionStr) {
      router.push("/")
      return
    }
  ...
  const handleLogout = () => {
    localStorage.removeItem("module_session_billing")
    router.push("/")
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-6">
        <div className="text-center py-20">
          ...
          <p className="text-muted-foreground">Sistema de facturación en desarrollo</p>
        </div>
      </main>
    </div>
  )
