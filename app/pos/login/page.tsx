...
export default function POSLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // Demo credentials: admin/admin or mesero/mesero
      if (
        (credentials.username === "admin" && credentials.password === "admin") ||
        (credentials.username === "mesero" && credentials.password === "mesero")
      ) {
        const role = credentials.username === "mesero" ? "mesero" : "cajero"
          ...
          <CardContent>
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="text-xs text-muted-foreground text-center pt-4 border-t">
                ...
                <p>Usuario: mesero | Contraseña: mesero</p>
              </div>
            </form>
          </CardContent>
        </Card>
